...
VirtualFileSystem::VirtualFileSystem()
	: CacheTable(nullptr), CacheBuffer(nullptr)
{
...
bool VirtualFileSystem::Mount(const char* FileName)
{
	...
	if( FileStream )
	{
		...
		for( size_t i = 0; i < BlockCount; i++ )
		{
			...
			{
				if( CacheTable->TableEntries[i & 0x1FF].Checksum
					!= CacheBuffer->Checksum(false) )
				{
			...
			{
				if( CacheTable->TableEntries[0].Checksum
					!= CacheTable->Checksum(true) )
				{
...
bool VirtualFileSystem::GetEntry(const char* Path, FileEntry& Entry)
{
	if( FileStream )
	{
		...
		{
			if( std::strcmp(CurToken, CacheBuffer->FATEntries[CurEntry].Name)
				== 0 )
			{
				...
				if( CacheBuffer->FATEntries[CurEntry].Type
					!= VirtualFileEntry::EntryType::Folder )
				{
					// Entry is not a folder, cant go further
					return false;
				}
				GetBlock(
					CacheBuffer->FATEntries[CurEntry].Block, CacheBuffer.get()
				);
				CurEntry = 0;
				continue;
			}
...
bool VirtualFileSystem::Read(size_t Offset, size_t Size, void* Destination)
{
	if( FileStream )
	{
		...
		{
			size_t CurBlock
				= Offset / FileSystemBlock::BlockSize; // Nearest Block Offset
			size_t CurBlockOffset
				= Offset % FileSystemBlock::BlockSize; // Offset within Block
...
void VirtualFileSystem::IterateCanvas(CanvasVisitor& Visitor)
{
	if( FileStream )
	{
		...
		Visitor.VisitCanvasBegin(
			Canvas.Read<uint32_t>(), Canvas.Read<uint32_t>()
		);
...
void VirtualFileSystem::VisitBlock(
	size_t BlockNumber, FileSystemVisitor& Visitor
)
{
...
bool VirtualFileSystem::GetBlock(size_t BlockNum, FileSystemBlock* Block)
{
	if( BlockNum < BlockCount )
	{
		if( BlockNum & 0x1FF ) // Block is data
		{
			...
			{
				Key = CacheTable->TableEntries[BlockNum - NearestTable]
						  .Checksum;
			}
			else // Cache Miss
			{
			...
			FileStream.read(
				reinterpret_cast<char*>(Block->u8), FileSystemBlock::BlockSize
			);
