...
```cpp
uint64_t Hash(const char* MachineID)
{
    uint8_t StringBlock[256];
    
    const char* ReadPoint = MachineID;
    for(size_t i = 0; i < 256; i++)
    {
        StringBlock[i] = *ReadPoint;
    ...
    {
        uint32_t Temp;
        uint32_t CurUpper = UpperHash + reinterpret_cast<uint32_t*>(StringBlock)[i & 63];
        uint32_t CurLower = LowerHash + reinterpret_cast<uint32_t*>(StringBlock)[(i + 1) & 63];
        for( size_t j = 0; j < 4; j++ )
        {
            CurUpper = CurLower + ((CurUpper << CurLower) | (CurUpper >> (32- CurLower)));
            Temp = CurUpper + ((CurLower << CurUpper) | (CurLower >> (32 - CurUpper)));
            CurLower = CurUpper + ((CurLower << CurUpper) | (CurLower >> (32 - CurUpper)));
        }
        LowerHash = Temp ^ Temp1;
        UpperHash ^= CurUpper;
        Temp1 ^= Temp;
    }
    return LowerHash | (uint64_t(UpperHash) << 32);
}
```

The resulting hash for the above formatted string is `a1541b366925e034` which would make the filename `.a1541b366925e034`.

The filename is generated by taking the hash and formatting it with the [printf](http://en.cppreference.com/w/cpp/io/c/fprintf) format `/%s.%016I64x`. The first parameter usually being a blank string.
